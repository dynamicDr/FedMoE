#!/bin/bash
# CIFAR-10 / ResNet 扫参，只跑两种方法：
#   Ours + Ours          --select ours    --merge ours
#   Post-hoc + Ours      --select posthoc --merge ours
#
# 网格：专家数 {4, 8, 16} × 骨干宽度系数 {0.5, 1} = 6 组配置。
# 每种配置两种方法各提交一次，共 12 个任务。
...
#   bash run_cifar10_ours_sweep.sh
#   bash run_cifar10_ours_sweep.sh --dry-run
# 查看 / 取消：
#   squeue -u "$USER"
#   scancel <jobid>
...
echo
echo "共 ${n_jobs} 个任务（6 组配置 × 2 种方法）-> 分区=${PARTITION}, GRES=${GRES}, 账号=${ACCOUNT}"
