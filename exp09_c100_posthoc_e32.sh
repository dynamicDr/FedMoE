#!/bin/bash
# exp09：CIFAR-100 Post-hoc，32 专家，带宽 [10, 20]，K 只有 1 或 2。
# 本地训练对全部专家做前向和反传，再按门控分数只上传 K 个。
# 其余与 exp01 相同：Top-2 路由、10 客户端、Dirichlet 0.1、100 轮、
# 本地 2 epoch、聚合 ours。只跑 ResNet 和 VGG11。
#
# 用法（在登录节点执行，不要在计算节点上再 sbatch）：
#   bash exp09_c100_posthoc_e32.sh
#   bash exp09_c100_posthoc_e32.sh --dry-run
...
for jspec in "${JOBS[@]}"; do
    ...
    name="${EXP_ID}-c100-${btag}-e${NUM_EXPERTS}-k12-posthoc"
...
echo
echo "共 ${n_jobs} 个任务（CIFAR-100 / Post-hoc / 32 专家 / 带宽 [${BW_MIN}, ${BW_MAX}]：ResNet, VGG11）-> 分区=${PARTITION}, GRES=${GRES}, 账号=${ACCOUNT}, 时限=${TIME}"
