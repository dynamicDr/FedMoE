#!/bin/bash
# exp03：CIFAR-100 / ResNet 的规模消融。每个条件一张图，图内 5 条 baseline。
# 不重跑主实验点（带宽 [10, 80]、8 专家、10 终端），该点已在 exp01。
# 新增 9 个条件 × 5 方法 = 45 个任务。
...
#   [1, 50]   --bw_min 1  --bw_max 50
#   [20, 80]  --bw_min 20 --bw_max 80
# 专家数（--bw_max -1，上界随专家数放大；10 终端）：4、16、32
# 终端数（8 专家、带宽 [10, 80]）：20、30、50
#
# 用法（在登录节点执行，不要在计算节点上再 sbatch）：
#   bash exp03_c100_resnet_scalability.sh
#   bash exp03_c100_resnet_scalability.sh --dry-run
# 查看 / 取消：
#   squeue -u "$USER"
#   scancel <jobid>
...
echo
echo "共 ${n_jobs} 个任务（9 个新条件 × 5 方法）-> 分区=${PARTITION}, GRES=${GRES}, 账号=${ACCOUNT}"
